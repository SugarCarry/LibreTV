// 全局变量
let selectedAPIs = JSON.parse(localStorage.getItem('selectedAPIs') || '["dyttzy", "ruyi", "bfzy", "tyyszy", "heimuer", "ffzy", "huya", "ckzy", "ikun", "wwzy", "zy360", "wolong", "hwba", "jisu", "dbzy", "mozhua", "mdzy", "jkun", "bwzy", "souav", "r155", "lsb", "huangcang", "zuid", "yutu","yinghua","baidu","wujin"]'); // 默认选中黑木耳和豆瓣资源
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!localStorage.getItem('hasInitializedDefaults')) {
        // 仅选择天涯资源、暴风资源和如意资源
        selectedAPIs = ["dyttzy", "ruyi", "bfzy", "tyyszy", "heimuer", "ffzy", "huya", "ckzy", "ikun", "wwzy", "zy360", "wolong", "hwba", "jisu", "dbzy", "mozhua", "mdzy", "jkun", "bwzy", "souav", "r155", "lsb", "huangcang", "zuid", "yutu","yinghua","baidu","wujin"];
...
function resetSearchArea() {
    ...
    try {
        window.history.pushState(
            {}, 
            `BestTV - 免费在线视频搜索与观看平台`, 
            `/`
        );
        // 更新页面标题
        document.title = `BestTV - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function search() {
    ...
    try {
        ...
        try {
            ...
            // 更新页面标题
            document.title = `搜索: ${query} - BestTV`;
        } catch (e) {
            console.error('更新浏览器历史失败:', e);
            // 如果更新URL失败，继续执行搜索
        }
...
async function exportConfig() {
    ...
    config['name'] = 'BestTV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'BestTV-Settings_' + times + '.json');
}
